...
## 商談創出AI(Sales AIモジュール)

会社アカウントでログイン後、ダッシュボードの「商談創出AIを開く」から利用できる追加モジュールです。
「営業マンを増やす前に、商談を増やす」という考え方のもと、以下を一連の流れで提供します。

企業登録 → AI営業・集客診断 → 理想顧客(ICP)設計 → 見込み企業(リード)登録・AIスコアリング → AIアプローチ文生成 → リード管理 → ダッシュボード

- `/dashboard/sales-ai`：今月のサマリー(新規リード数・有望リード数・商談候補〜受注件数・受注金額)とAIによるボトルネック分析
- `/dashboard/sales-ai/profile`：自社の企業情報を登録し、集客・営業・リードの3領域をAIが100点満点で診断
- `/dashboard/sales-ai/icp`：企業情報から「最も売れやすい顧客像」とその理由をAIが生成
- `/dashboard/sales-ai/leads`：見込み企業を登録・一覧管理(ステータスは 未接触→アプローチ済→反応あり→興味あり→商談候補→商談→提案→受注/失注)
- `/dashboard/sales-ai/leads/[id]`：見込み度スコアの根拠を確認し、メール・問い合わせフォーム・電話・初回商談トークのアプローチ文をAIが生成(自動送信はせず、生成・コピーまで)

利用にはSupabaseに `supabase/schema.sql` を実行済みであることが必要です(`sales_profiles` / `sales_diagnoses` / `icps` / `leads` / `approach_texts` の5テーブルが追加されます)。

診断・ICP・アプローチ文の生成は、`.env.local` に `ANTHROPIC_API_KEY` を設定するとAnthropic(Claude)を使ってより自然な文章を生成します。**未設定でもルールベースのロジックで全機能がそのまま動作します**(スコアリングは常にルールベースで行われ、AIは文章の質を上げる用途にのみ使われます)。
...
src/app/dashboard/sales-ai/    商談創出AI(診断・ICP・リード管理・ダッシュボード)
src/lib/salesAi/               商談創出AIのスコアリング・診断ロジック(ルールベース)・AI連携
